...
ENTITLEMENTS     = Resources/BarMaster.entitlements
MAS_ENTITLEMENTS = Resources/BarMaster.mas.entitlements
MAS_PKG          = build/$(APP_NAME).pkg
MAS_PROFILE      ?= Resources/BarMaster_MAS.provisionprofile
...
SIGN_ID := $(shell security find-certificate -c "BarMaster Dev" >/dev/null 2>&1 && echo "BarMaster Dev" || echo -)

ifdef MAS
SWIFT_FLAGS += -Xswiftc -DBARMASTER_MAS
endif
...
DIST_DMG     = build/BarMaster-$(DIST_VERSION).dmg
DIST_JSON    = build/BarMaster-$(DIST_VERSION).json

dist: icon
	@echo "── Direct-distribution build: BarMaster $(DIST_VERSION) ──"
	...
	@echo "Upload:"
	@echo "  wrangler r2 object put anti-ltd-binaries/binaries/barmaster.dmg  --file $(DIST_DMG)"
	@echo "  wrangler r2 object put anti-ltd-binaries/binaries/barmaster.json --file $(DIST_JSON) --content-type application/json"
...
dist-manifest:
	...
	NOTES=$${BARMASTER_RELEASE_NOTES:-"Initial release."}; \
	printf '{\n  "version": "%s",\n  "releasedAt": "%s",\n  "notes": "%s",\n  "minOS": "macOS %s",\n  "sha256": "%s",\n  "size": %d\n}\n' \
