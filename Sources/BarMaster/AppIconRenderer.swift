...
enum AppIconRenderer {
    static let symbolName = "menubar.rectangle"
    ...
    private static func draw(in cg: CGContext, size: CGFloat) {
        ...
        // Deep indigo-to-midnight gradient, light from top-left.
        cg.saveGState()
        cg.addPath(bg)
        cg.clip()
        ...
        // Centered SF Symbol glyph, white, ~52% of the canvas.
        ...
        cg.addPath(bg)
        cg.clip()
        cg.setShadow(offset: CGSize(width: 0, height: -size * 0.012), blur: size * 0.02,
